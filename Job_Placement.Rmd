...
Based on this dataset, the first important question we must answer is which metrics we will use to judge fairness. Demographic parity, while nice in concept, doesn't seem like the best metric to judge a dataset.
...
Males and females in our dataset are not hired at equal rates and do not have equal measurements and statistics. Predicitve value parity would benefit the employers hiring candidates but not necessarily the candidates. 
...
```{r}
plot(x = Jobslr$ssc_percentage, y = Jobslr$Placed)

glm.fit = glm(Placed ~ ssc_percentage + mba_percent + degree_percentage + gender, family = binomial, data = Jobslr)
lines(Jobslr$ssc_percentage, glm.fit$fitted.values)
```
...
par(pty = "s")
# Plot ROC curve
roc(Jobslr$Placed, glm.fit$fitted.values, plot = T, legacy.axes = T, percent = T, 
    xlab = "False Positive Percentage", ylab = "True Positive Percentage",
    col = "dodgerblue", lwd = 4, print.auc = T)
```
```{r}
# Obtain thresholds from ROC curve
roc.info <- roc(Jobslr$Placed, glm.fit$fitted.values, legacy.axes = T)
...
Based on the confusion matrices produced for males and females respectively, we see that the model gives equal sensitivity rates, 89%, amongst genders with the adjusted threshold values. This new model satisfies equalized odds while generating a reasonable level of accuracy, 86.33% for males and 82.89% for females.
